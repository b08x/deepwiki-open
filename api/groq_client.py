...
class GroqClient(ModelClient):
    ...
    Groq provides ultra-fast LLM inference using their LPU (Language Processing Unit) technology.
    Their API is compatible with OpenAI's format for easy integration.

    Visit https://console.groq.com/docs for more details.
    ...
    Example:
        ...
        generator = adal.Generator(
            model_client=client,
            model_kwargs={"model": "llama-3.3-70b-versatile"}
        )
        ```

    Supported Models:
        - llama-3.3-70b-versatile: Latest Llama 3.3 70B model (fast)
        - llama-3.1-70b-versatile: Llama 3.1 70B model
        - llama-3.1-8b-instant: Llama 3.1 8B model (ultra-fast)
    ...
    def convert_inputs_to_api_kwargs(
        ...
        """Convert AdalFlow inputs to Groq API format."""
        ...
        if model_type == ModelType.LLM:
            ...
            if isinstance(input, str):
                ...
                raise ValueError(
                    f"Unsupported input format for Groq: {type(input)}")

            log.debug(f"Messages for Groq: {messages}")
            ...
            if "model" not in api_kwargs:
                api_kwargs["model"] = "llama-3.3-70b-versatile"
    ...
    def parse_chat_completion(self, completion: Dict) -> GeneratorOutput:
        """Parse Groq chat completion response to GeneratorOutput.

        Args:
            completion: Raw API response from Groq
        ...
        try:
            ...
            log.error(f"Error parsing Groq completion: {e}")
            return GeneratorOutput(
                data=None,
    ...
    def call(self, api_kwargs: Dict, model_type: ModelType) -> GeneratorOutput:
        ...
        try:
            log.debug(f"Calling Groq API with model: {api_kwargs.get('model')}")
            ...
            log.error(f"Groq API request failed: {str(e)}")
            return GeneratorOutput(
                data=None,
                error=f"API request failed: {str(e)}"
            )
        except Exception as e:
            log.error(f"Unexpected error calling Groq API: {str(e)}")
            return GeneratorOutput(
                data=None,
    ...
    async def acall(self, api_kwargs: Dict, model_type: ModelType) -> GeneratorOutput:
        """Make asynchronous call to Groq API.
        ...
        try:
            log.debug(f"Async calling Groq API with model: {api_kwargs.get('model')}")
            ...
            log.error("Groq API async request timed out")
            return GeneratorOutput(
                data=None,
                error="Request timed out"
            )
        except aiohttp.ClientError as e:
            log.error(f"Groq API async request failed: {str(e)}")
            return GeneratorOutput(
                data=None,
                error=f"API request failed: {str(e)}"
            )
        except Exception as e:
            log.error(f"Unexpected error in async Groq API call: {str(e)}")
            return GeneratorOutput(
                data=None,
                error=f"Unexpected error: {str(e)}"
            )
