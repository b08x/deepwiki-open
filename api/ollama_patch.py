    ...
    def __call__(self, documents: Sequence[Document]) -> Sequence[Document]:
        ...
        for i, doc in enumerate(
            ...
            try:
                ...
                # Truncate text to avoid "input length exceeds the context length" error
                text_to_embed = doc.text
                if not text_to_embed or text_to_embed.strip() == "":
                    logger.warning(f"Document has empty text, skipping")
                    continue
                ...
                if len(text_to_embed) > 8000:
                    ...
                    logger.warning(
                        f"Truncating document '{file_path}' for Ollama embedding (length: {len(doc.text)} -> 8000)"
                    )

                # Use .call() directly to avoid Parameter wrapping if any
                result = self.embedder.call(input=text_to_embed)
